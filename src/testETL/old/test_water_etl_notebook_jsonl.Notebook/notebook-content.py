...
def write_jsonl(
    ...
    with _open_outfile(out_path) as fh:
        for event in events:
            try:
                # Ensure record_id is a string (spec requires string)
                rid = event.get("record_id")
                if rid is not None:
                    s = str(rid).strip()
                    if s:
                        event["record_id"] = s  # keep as string
            except Exception as e:
                # Log or ignore any unexpected error
                print(f"Warning: Could not process record_id for event: {e}")
