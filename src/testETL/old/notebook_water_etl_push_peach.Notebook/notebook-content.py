...
def _build_pes(row: Mapping[str, Any], events: List[Mapping]) -> Dict[str, Any]:
    ...
    raw_record_id = row.get("AUTHORIZATION_ID") or row.get("PROJECT_ID")  # CHANGED
    record_id_value = _to_int_no_decimal(raw_record_id)  # CHANGED
    if record_id_value is None:  # CHANGED
        record_id_value = "Error record_id"  # CHANGED
    ...
    return {
        ...
        "record_id": record_id_value,  # CHANGED: now int if convertible
        "record_kind": "Permit",
        "on_hold_event_set": [],
...
OUTPUT_FILENAME = "events_trackingnumber.jsonl"  # CHANGED: no .gz, plain JSONL
