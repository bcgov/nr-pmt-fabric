...
# CONFIGURATION
# ----------------------------------------------------------------
API_URL = "https://nr-peach-test-main-api.azurewebsites.net/api/events/upload"
...
def list_existing_files() -> list[str]:
    ...
    file_df = spark.read.format("binaryFile").option("pathGlobFilter", "*.jsonl.gz").load(ABFS_DIR)
...
def stream_jsonl_to_api(file_path: Path, api_url: str, chunk_size: int = 1000):
    """Stream JSONL.GZ file to API in batches."""
    ...
    with gzip.open(file_path, "rt", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            if len(batch) >= chunk_size:
                ...
                print(f"→ Sent {len(batch)} | Status {resp.status_code}")
                total += len(batch)
                batch.clear()
        if batch:
            resp = requests.post(api_url, json=batch)
            print(f"→ Final {len(batch)} | Status {resp.status_code}")
            total += len(batch)
