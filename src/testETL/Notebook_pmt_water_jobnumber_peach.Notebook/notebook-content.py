...
#
# This version reads from a Fabric table (query_jobnumber_piesid)
...
def _build_event(
    ...
    if stat_info:
        # status
        stat = (
            stat_info.get("STATUS")
            or stat_info.get("Status")
            or stat_info.get("status")
        )
        if stat:
        ...
        # status_code
        scode = (
            stat_info.get("status_code")
            or stat_info.get("Status_code")
            or stat_info.get("STATUS_CODE")
        )
        if scode:
...
def _build_pes(row: Mapping[str, Any], events: List[Mapping]) -> Dict[str, Any]:
    ...
    return {
        ...
        "version": "0.1.0",
        "kind": "Record",
        "system_id": row.get("system_id", "ITSM-6197"),
...
TABLE_NAME = "testETL_lakehouse.query_jobnumber_piesid"
